                    ...
                    for file in set(files.split('.')[0] for files in os.listdir(base_dir + "nose")):
                        ...
                        for file in set(files.split('.')[0] for files in os.listdir(base_dir + "mouth")):
                            ...
                            for file in set(files.split('.')[0] for files in os.listdir(base_dir + "hair")):
                                ...
                                random_rarity = gen_unique_rarity(background_json, head_json, ears_json, brows_json, eye_json, nose_json, mouth_json, hair_json)
                                keys = combine_keys(background_json, head_json, ears_json, brows_json, eye_json, nose_json, mouth_json, hair_json)

                                oFile_json.write('{'+f'title: {title},keywords: {keys},'+f'rarity: {random_rarity},'+f'price: {random_price * random_rarity}, '+'}')

                                print(background_json['title'])

                                with open("Insert_Assets.sql", 'a') as sql_file:
                                    encoded_image = base64.b64decode(svg_image)
                                    sql_file.write(sql_template.format(v1=random_id, v2=random_price, v3=keys, v4=encoded_image, v5=random_rarity))

                                # Open Insert_Assets.sql and then put insert statements there
